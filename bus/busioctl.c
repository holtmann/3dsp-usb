...
int tdspbus_ioctl(struct file *pfile, unsigned int cmd, unsigned long arg )
#else
...
//	Bus_DebugPrint( "%s is called!\n", __FUNCTION__ );
    ...
    {
        Bus_DebugPrint( "%s %d: is called,param verify failed!file=0x%p.\n", __FUNCTION__, __LINE__, pfile );
	return  - EFAULT;
    }
    ...
    {
	Bus_DebugPrint("%s %d: %d cmd = %x\n", __FUNCTION__, __LINE__, fdoData->DevicePnPState, cmd - IOCTL_BUSENUM_PLUGIN_HARDWARE);
	return  - EFAULT;
    }

    if ( fdoData->DevicePowerState != PowerDeviceD0 )
    {
        Bus_DebugPrint( "%s %d: is called but the power state of fdo is not actived!fdoData=0x%p.\n", __FUNCTION__, __LINE__, fdoData );
	return  - EFAULT;
    }
    ...
    switch ( cmd )
    {
        case IOCTL_BUSENUM_POWER_SOURCE_AC:
	//	tdspbus_trace();
		{
			UCHAR i;
			...
			for (i = 0; i < BUS_DEVICE_TOTAL_COUNT; i++)
			{
				...
				{
					(fdoData->DeviceData[i].pDeviceInterface->power_source_notify)(fdoData->DeviceData[i].pDeviceInterface,POWER_SOURCE_AC);
        ...
        case IOCTL_BUSENUM_POWER_SOURCE_BATTERY:
	//	tdspbus_trace();
		{
			UCHAR i;
			...
			for (i = 0; i < BUS_DEVICE_TOTAL_COUNT; i++)
			{
				if (fdoData->DeviceData[i].pDeviceInterface != NULL)
				{
					(fdoData->DeviceData[i].pDeviceInterface->power_source_notify)(fdoData->DeviceData[i].pDeviceInterface,POWER_SOURCE_BATTERY);
