...
#define BUS_STRING_VERSION_ID "BusU101209"
...
{
    .owner = THIS_MODULE, .unlocked_ioctl = tdspbus_ioctl, .open = tdspbus_open, .release = tdspbus_release,
};
#else	
...
int register_tdspbus_usbdev( PTDSPBUS_INTERFACE pInterface )
{
    ...
    if (	( pInterface->open == NULL ) 
	||	( pInterface->close == NULL ) 
	||	( pInterface->power_source_notify == NULL ))
    {
