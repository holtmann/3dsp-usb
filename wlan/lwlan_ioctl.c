...
static int _ioctl_priv_get_driver_version
	(	struct net_device *dev,
		struct iw_request_info *info,
		union iwreq_data *wrqu, char *extra)
{
	sprintf(extra, "%s", WLAN_DRV_VERSION);
	wrqu->data.length = strlen(extra) + 1;
	return 0;
}

static int _ioctl_priv_get_tx_power
	(	struct net_device *dev,
		struct iw_request_info *info,
		union iwreq_data *wrqu, char *extra)
{
	void * pAdap  = lwlan_get_netdev_priv(dev);
	sprintf(extra, "%d%%", Adap_Get_Txpower(pAdap));
	wrqu->data.length = strlen(extra) + 1;
	return 0;
}

static const char*	domain_char[] = {"USA", "Europe", "Japan", "Reset of World"};

static int _ioctl_priv_set_domain(struct net_device *dev,
				    struct iw_request_info *info,
				    union iwreq_data *wrqu, char *extra)
{
	void * pAdap  = lwlan_get_netdev_priv(dev);
	...
	switch (mode)
	{
		...
		case 3://DRIVERREGION_RESTOFWORLD:
			printk("%s: set domian to %s\n", __FUNCTION__, domain_char[mode]);
			break;
		default:
			printk( "[%s]    Unknown country region[%x] !!! ",
							__FUNCTION__, mode);
			return -EINVAL;
	}

	Adap_Set_Domain(pAdap, (unsigned char)(mode));

	return 0;
}


static int _ioctl_priv_get_domain(struct net_device *dev,
				    struct iw_request_info *info,
				    union iwreq_data *wrqu, char *extra)
{
	void * pAdap  = lwlan_get_netdev_priv(dev);
	...
	sprintf(extra, "%s (%d)", domain_char[Adap_Get_Domain(pAdap)], Adap_Get_Domain(pAdap));
	wrqu->data.length = strlen(extra) + 1;
...
static const iw_handler _wdev_iwpriv_handler[] = {			
	NULL,
	(iw_handler) _ioctl_priv_get_driver_version,			
	NULL,
	(iw_handler) _ioctl_priv_get_tx_power,				
	NULL,	
	(iw_handler) _ioctl_priv_set_domain,				
	NULL,	
	(iw_handler) _ioctl_priv_get_domain,
};	
...
static const struct iw_priv_args _wdev_iwpriv_args[] = {
	...
	{},
	{DSP_PARAM_DRIVER_VER,	 	0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | 256, "driver_version" },
	{},
	{DSP_PARAM_GET_TXPWR,		0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | 16,  "get_tx_power" },
	{},
	{DSP_PARAM_SET_DOMAIN,	 	IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "domain"}, 
	{},
	{DSP_PARAM_GET_DOMAIN,	 	0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | 32, "get_domain" },
};
